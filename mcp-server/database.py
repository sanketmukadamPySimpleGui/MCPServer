...
class DatabaseConnector(ABC):
    ...
    def get_type(self) -> str:
        """Returns the type of the database connector (e.g., 'mongodb', 'sqlite')."""
        pass
    ...
    @abstractmethod
    def get_table_schema(self, collection: str) -> dict[str, Any]:
        pass
    ...
    @abstractmethod
    def find_documents(self, collection: str, filter: dict, projection: Optional[dict] = None, limit: int = 50) -> dict[str, Any]:
        pass
    
    @abstractmethod
    def count_documents(self, collection: str, filter: Optional[dict] = None) -> int:
        pass
...
class SQLiteInMemoryConnector(DatabaseConnector):
    ...
    def get_table_schema(self, collection: str) -> dict[str, Any]:
        ...
        try:
            ...
            if not columns:
                return {"error": f"Table '{collection}' not found."}
...
class MongoDbConnector(DatabaseConnector):
    ...
    def get_table_schema(self, collection: str) -> dict[str, Any]:
        ...
        if not sample_doc:
            return {"error": f"Collection '{collection}' is empty or does not exist."}
        ...
        def get_type_name(value):
            if isinstance(value, list):
                ...
                return {k: get_type_name(v) for k, v in value.items()}
            return type(value).__name__

        return {"schema": {key: get_type_name(value) for key, value in sample_doc.items()}}
    ...
    def find_documents(self, collection: str, filter: dict, projection: Optional[dict] = None, limit: int = 50) -> dict[str, Any]:
        if self.db is None:
            return {"error": "Not connected to database."}
        try:
            # Add projection to only retrieve name and email address
            if projection is None:
                projection = {"name": 1, "email": 1}
            projection["_id"] = 0
            
            documents = list(self.db[collection].find(filter, projection).limit(limit))
            
            # The previous version had a bug where it would convert ObjectId to string, but it was not returning a useful error message.
            # This is now fixed, and the method will return a meaningful error.
            if not documents:
                return {"error": "No documents found with the given criteria."}
...
class DatabaseManager:
    ...
    def _parse_env_configs(self):
        ...
        logging.info("Parsing environment variables for database connections (DB_CONN_*)...")
        
        # A simple, hardcoded in-memory SQLite connector for the demo
        self.add_connector("sqlite_demo", SQLiteInMemoryConnector())
        logging.info("Added hardcoded 'sqlite_demo' connection.")

        db_configs: Dict[str, Dict[str, str]] = {}
        db_env_vars = {k: v for k, v in os.environ.items() if k.startswith("DB_CONN_")}
    
        if not db_env_vars:
            logging.warning("No DB_CONN_* environment variables found.")
            return

        logging.info(f"Found {len(db_env_vars)} DB_CONN_* environment variables: {list(db_env_vars.keys())}")

        for key, value in db_env_vars.items():
            try:
                conn_name_parts, suffix = key.removeprefix("DB_CONN_").rsplit('_', 1)
                conn_name = conn_name_parts.lower()
                if conn_name not in db_configs:
                    db_configs[conn_name] = {}
                db_configs[conn_name][suffix.lower()] = value
            except ValueError:
                logging.warning(f"Skipping malformed environment variable: {key}")
        ...
        for conn_name, config in db_configs.items():
            ...
            if db_type == 'mongodb' and uri and db_name:
                try:
                    self.add_connector(conn_name, MongoDbConnector(uri, db_name))
                    logging.info(f"Successfully added MongoDB connection: '{conn_name}'")
                except ImportError as e:
                    logging.error(f"Failed to add MongoDB connector '{conn_name}': {e}. Is pymongo installed?")
            else:
                logging.warning(f"Skipping connection '{conn_name}': Invalid or incomplete configuration.")

    def add_connector(self, name: str, connector: DatabaseConnector) -> None:
        """Adds a named database connector to the manager."""
        self._connectors[name] = connector
        
    def connect_all(self) -> None:
        """Establishes connections for all registered connectors."""
        for name, connector in self._connectors.items():
            try:
                connector.connect()
            except Exception as e:
                logging.error(f"Failed to connect to database '{name}': {e}", exc_info=True)
    ...
    def disconnect_all(self) -> None:
        ...
        for name, connector in self._connectors.items():
            try:
                ...
                logging.error(f"Failed to disconnect from database '{name}': {e}")
    ...
    def get_connector_info(self, name: str) -> Optional[dict]:
        """Returns a dictionary with information about a specific connector."""
        connector = self.get_connector(name)
        if connector:
            return {"name": name, "type": connector.get_type()}
        return None